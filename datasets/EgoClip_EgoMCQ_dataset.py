...
# I'll use it mainly for feature extraction
# for the rest of the experiments I"ll be using pre-extracted features


class EgoClip_EgoMCQ(DatasetBase):
    def _load_metadata(self, split):
        ...
        if self.split == 'train':
            ...
            self.metadata['segment_id'] = self.metadata['video_uid'] + '_' + (
                        self.metadata['narration_time'] // self.chunk_sec).astype(str)
    ...
    def _get_train_item(self, item):
        ...
        if self.video_params.get('disable', False):
            ...
            final, im_sz, crop_params, _, seconds = self._get_video_frames(video_fp, video_sec, bound_sec, boxes=(
                box if self.crop_with_boxes else None))
        ...
        final_neg, im_sz_neg, crop_params_neg, _, seconds_neg = self._get_video_frames(video_fp_neg, video_sec_neg,
                                                                                       bound_sec_neg,
                                                                                       boxes=(
                                                                                           box_neg if self.crop_with_boxes else None))
