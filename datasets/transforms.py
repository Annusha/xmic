...
from datasets.box_ops import box_cxcywh_to_xyxy
import torch
...
def custom_img_crop(imgs, boxes, pred=False):
    ...
    if torch.min(im_sz[0] - newy1, long_side) < 1 or torch.min(im_sz[1] - newx1, long_side) < 1:
        return imgs, torch.tensor([0, 0, 0, 0])
    new_imgs = F.crop(imgs, newy1, newx1, torch.min(im_sz[0] - newy1, long_side),
                      torch.min(im_sz[1] - newx1, long_side))
    crop_params = torch.stack(
        [newy1, newx1, torch.min(im_sz[0] - newy1, long_side), torch.min(im_sz[1] - newx1, long_side)])
