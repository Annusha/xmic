...
from datasets.transforms import init_transform_dict, init_video_transform_dict

# from utils.utils import img_denorm

@DATASET_REGISTRY.register()
class EgoClip_EgoMCQ_Features_Dataset(DatasetBase):
    def __init__(self, cfg):
        self.cfg = cfg

        n_frames = cfg.DATALOADER.FRAMES_PER_SEGMENT
        ...
        subsample = 'mcq' if tsfm_split == 'val' else 1

        kwargs = {
            'dataset_name': 'EgoClip',
            'text_params': text_params,
            'video_params': video_params,
            'data_dir': cfg.DATA.PATH_PREFIX,
            'meta_dir': cfg.DATA.PATH_TO_DATA_DIR,
            'split': tsfm_split,
            'tsfms': tsfm,
            'cut': None,
            'subsample': subsample,
            ...
            'slice_idx': 0,
            'n_slice': 1,
            'hard_neg': cfg.DATALOADER.EGOCLIP.WITH_NEGS

        }

        dataset_train = EgoClip_EgoMCQ_Features(**kwargs)
        ...
        kwargs = {
            ...
            'text_params': text_params,
            'video_params': video_params,
            'data_dir': cfg.DATA.PATH_PREFIX_MCQ,
            'meta_dir': cfg.DATA.PATH_TO_DATA_DIR,
            'split': tsfm_split,
            'tsfms': tsfm,
            'cut': None,
        ...
        dataset_val = EgoClip_EgoMCQ_Features(**kwargs)

        anno_path = cfg.DATA.PATH_TO_DATA_DIR
        nouns_tax = pd.read_csv(os.path.join(anno_path, 'narration_noun_taxonomy.csv'))
        nouns_tax['group'] = nouns_tax['group'].apply(eval)
        verb_tax = pd.read_csv(os.path.join(anno_path, 'narration_verb_taxonomy.csv'))
        verb_tax['group'] = verb_tax['group'].apply(eval)
        nouns_l2c = {}
        verbs_l2c = {}
        for row in nouns_tax.iterrows():
            nouns_l2c[row[0]] = row[1].group[0]
        for row in verb_tax.iterrows():
            verbs_l2c[row[0]] = row[1].group[0]
        ...
        noun_cls = [v for k,v in sorted(nouns_l2c.items(), key=lambda x: x[0])]
        verb_cls = [v for k,v in sorted(verbs_l2c.items(), key=lambda x: x[0])]
        self._classnames = {
            'noun': noun_cls,
            'verb': verb_cls
        }
        self.n_test_classes = {'noun': 582, 'verb': 118}
        super().__init__(train_x=dataset_train, val=dataset_val, test=dataset_val)
    ...
    def get_num_classes(self, data_source=None):
        ...
        # noun, verb, action
        # return [300, 97, 3806]
        return {'noun': 582, 'verb': 118}


class EgoClip_EgoMCQ_Features(TextVideoDataset):
    def _load_metadata(self):
        ...
        self.shift_around_narration = 4.0

        self.noun_pd = pd.read_csv(path_narration_noun)
        self.noun_pd['group'] = self.noun_pd['group'].apply(eval)
    ...
    def _get_video_path(self, sample):
        ...
        video_start_sec = max(float(narration_time) - self.shift_around_narration, 0)
        video_end_sec = max(float(narration_time) + self.shift_around_narration, 0)
        ...
        try:
            if os.path.isfile(video_fp[0]) and os.path.isfile(video_fp[1]):
                imgs, seconds = self.video_reader(video_fp[0], video_sec[0], end_second=video_sec[1],
                                                  clip_length=self.video_params['num_frames'], jitter=jitter)
                valid = 1
            else:
    ...
    def _get_rephrased_caption(self, sample, video_sec, caption):
        ...
        rephrased_texts = [''] * 5
        rephrased_texts = [caption, caption, caption, caption, caption]
    ...
    def _get_train_item(self, item):
        ...
        video_fp = f'{self.data_dir}/{sample.video_uid}/{sample.narration_source}_ind{sample.narration_ind}.npy'
        while not os.path.exists(video_fp):
            item = item + 1
            item = item % len(self.metadata)
            sample = self.metadata.iloc[item]
            video_fp, video_sec, bound_sec = self._get_video_path(sample)
        ...
        if self.video_params.get('disable', False):
            ...
            final = np.load(video_fp)
            idx = np.random.choice(range(0, len(final)))
            final = final[idx]
        ...
        meta_arr = [sample.video_uid, sample.clip_start, sample.clip_end, [0,0,0,0]]
        ...
        video_fp = f'{self.data_dir}/{sample_neg.video_uid}/{sample_neg.narration_source}_ind{sample_neg.narration_ind}.npy'

        while sample_negs.shape[0] != 1 and sample_neg['clip_start'] == sample['clip_start'] and counter < 10 and not os.path.exists(video_fp):
            sample_neg = sample_negs.sample(1).iloc[0]
            caption_neg, noun_vec_neg, verb_vec_neg = self._get_caption(sample_neg)
            counter += 1
            video_fp = f'{self.data_dir}/{sample_neg.video_uid}/{sample_neg.narration_source}_ind{sample_neg.narration_ind}.npy'
        ...
        video_fp = f'{self.data_dir}/{sample_neg.video_uid}/{sample_neg.narration_source}_ind{sample_neg.narration_ind}.npy'
        final_neg = np.load(video_fp)
        idx_neg = np.random.choice(range(0, len(final_neg)))
        final_neg = final_neg[idx_neg]
        ...
        meta_arr_neg = [sample_neg.video_uid, sample_neg.clip_start, sample_neg.clip_end,  [0,0,0,0]]
        ...
        return {'video': final, 'text': caption,
                ...
                'boxes': 0, 'boxes_neg': 0,
                'image_size': torch.tensor(0), 'image_size_neg': torch.tensor(0),
                'crop_params': [0,0,0,0], 'crop_params_neg': [0,0,0,0],
                'rephrased_text': rephrased_caption, 'rephrased_text_neg': rephrased_caption_neg,
                'data_item': item}

    def _get_train_item_w_neg(self, item):
        self.counter += 1
        item = item % len(self.metadata)
        sample = self.metadata.iloc[item]
        video_fp, video_sec, bound_sec = self._get_video_path(sample)
        ...
        try:
            ...
            video_fp += '111'  # such file definitely does not exist
        ...
        try:
            ...
            pseudo_neg = self._get_train_item_wo_neg(np.random.choice(range(len(self.metadata))))
            nouns_neg = pseudo_neg['nouns']
            meta_arr_neg = pseudo_neg['meta']
            caption_neg = pseudo_neg['text']
            noun_vec_neg = pseudo_neg['noun_vec']
            verb_vec_neg = pseudo_neg['verb_vec']
            rephrased_caption_neg = pseudo_neg['rephrased_text']
            final_neg = pseudo_neg['video']
    ...
    def _get_train_item_wo_neg(self, item):
        self.counter += 1
        item = item % len(self.metadata)
        sample = self.metadata.iloc[item]
        video_fp, video_sec, bound_sec = self._get_video_path(sample)
    ...
    def _get_val_item(self, item):
        ...
        videoOptions = np.load(f'{self.data_dir}/{name}.npy')

        data = {'video': videoOptions,  # features
                'text': textQuery,
    ...
    def __getitem__(self, item):
        if self.split == 'train':
            if self.hard_neg:
                return self._get_train_item_w_neg(item)
            return self._get_train_item_wo_neg(item)
        elif self.split in ['val', 'test']:
