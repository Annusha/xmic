...
class ConfigParser:
    def __init__(self, args_parser, options='', timestamp=True, test=False, parse_from_string=None):
        ...
        if args.resume is None:
            ...
            config = read_yaml(resume_cfg_fname)
            if args.config is not None:
                config.update(read_yaml(Path(args.config)))

        # load config file and apply custom cli options
        self.config = _update_config(config, options, args)

        # set seed
        config['seed'] = config.get('seed', None)  # set None if it's not given
