...
class PromptLearner(nn.Module):
    ...
    # def forward(self, start=-1, end=-1, test=False, ctx_vis_features=None):
    #     ctx = self.ctx
    #     step = self.cls_step
    #     if self.n_cls > step:
    ...
    #         assert ctx.dim() == 2
    #         ctx = ctx.unsqueeze(0)
    #         ctx_shifted = (ctx + ctx_vis_features).unsqueeze(1).expand(-1, n_cls, -1,-1)
    ...
    #             prompts = torch.cat(
    #                 [
    #                     prefix,  # (b, n_cls, 1, dim)
    #                     ctx_shifted,     # (b, n_cls, n_ctx, dim)
    #                     suffix,  # (b, n_cls, *, dim)
    #                 ],
    #                 dim=2,
    #             )
    ...
    def forward(self, start=-1, end=-1, test=False, ctx_vis_features=None, **kwargs):
        ctx = self.ctx
        ...
        assert ctx.dim() == 2
        ctx = ctx.unsqueeze(0)
        ctx_shifted = (ctx + ctx_vis_features)
        prompts = []
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, device='cuda', test_classnames=None, egtea_classes=None):
        ...
        self.meta_net = nn.Sequential(OrderedDict([
            ...
            ("linear2", nn.Linear(self.hidden_size // 16, self.prompt_learner.ctx_dim))
        ]))
    ...
    def forward(self, image, test=False, **kwargs):
        ...
        ctx_image_features = self.temporal_forward(ctx_image_features, b=b, t=t)
        ...
        if self.prompt_learner.n_cls > self.prompt_learner.cls_step:
            ...
            prompts = self.prompt_learner(test=test, ctx_vis_features=ctx_image_features, **kwargs)
            ...
            logits = []
            for pts_i, img_i in zip(prompts, image_features):
                text_features = self.text_encoder(pts_i, tokenized_prompts)
                text_features = text_features / text_features.norm(dim=-1, keepdim=True)
                l_i = logit_scale * img_i @ text_features.t()
                logits.append(l_i)
...
class TemporalCoCoOp(TrainerXEpic):
