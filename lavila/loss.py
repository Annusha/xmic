...
class MaxMarginRankingLoss(nn.Module):
    ...
    def forward(self, outputs, weight=None):
        ...
        all_image_features = gather_from_all(image_features)
        all_text_features = gather_from_all(text_features)
        if 'visual_ctx_feat' in outputs:
            visual_ctx_feat = outputs['visual_ctx_feat']
            all_visual_ctx_feat = gather_from_all(visual_ctx_feat)
            all_text_features = all_text_features + all_visual_ctx_feat
            all_text_features = all_text_features / all_text_features.norm(dim=-1, keepdim=True)

            all_video_embed = all_image_features.unsqueeze(1)
            x = ((all_video_embed * all_text_features).sum(-1))
        else:
