...
def _build_transform_train(cfg, choices, target_size, normalize):
    ...
    if 'resize_wo_crop' in choices:
        tfm_train += [Resize((input_size, input_size), interpolation=interp_mode)]
    else:
    ...
    if 'no_to_tensor' in choices:
        print("no to torch tensor transform")
    else:
