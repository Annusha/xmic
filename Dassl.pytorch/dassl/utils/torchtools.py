...
def load_checkpoint(fpath, local_rank=-1):
    r"""Load checkpoint.
    ...
    print('MAP LOCATION', map_location, flush=True)
    if local_rank >= 0:
        map_location = 'cpu'
        print('LOAD TO CPU', flush=True)

    try:
        checkpoint = torch.load(fpath, map_location=map_location)
        # if map_location is None and local_rank > 0:
        #     checkpoint['state_dict'].cuda(local_rank)
        #     checkpoint['optimizer'].cuda(local_rank)
...
def resume_from_checkpoint(fdir, model, optimizer=None, scheduler=None, local_rank=-1):
    ...
    checkpoint = load_checkpoint(fpath, local_rank=local_rank)

    if local_rank >= 0:
        model.cpu()
        missing_name = 'prompt_learner.test_token_prefix'
        if missing_name in model.state_dict() and missing_name not in checkpoint["state_dict"]:
            output = model.load_state_dict(checkpoint["state_dict"], strict=False)
        else:
            if 'prompt_learner.token_prefix' in checkpoint['state_dict']:
                del checkpoint['state_dict']['prompt_learner.token_prefix']
                del checkpoint['state_dict']['prompt_learner.token_suffix']
            output = model.load_state_dict(checkpoint["state_dict"], strict=False)
            # output = model.load_state_dict(checkpoint["state_dict"]))
...
def load_pretrained_weights(model, weight_path):
    ...
    # missing_name = 'prompt_learner.test_token_prefix'
    # if missing_name in model.state_dict() and missing_name not in model_dict:
    #     output = model.load_state_dict(model_dict, strict=False)
    # else:
    #     if 'prompt_learner.token_prefix' in checkpoint['state_dict']:
    #         del checkpoint['state_dict']['prompt_learner.token_prefix']
    #         del checkpoint['state_dict']['prompt_learner.token_suffix']
    #         output = model.load_state_dict(checkpoint["state_dict"], strict=False)
    #     else:
    #         output = model.load_state_dict(checkpoint["state_dict"], strict=True)
